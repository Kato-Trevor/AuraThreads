...
const WordCloud = () => {
  ...
  useEffect(() => {
    const loadTopicCounts = async () => {
      try {
        setLoading((prev: any) => ({ ...prev, topics: true }));
        ...
        console.log("Loaded topic counts:", topicsWithCounts);
      } catch (error) {
        ...
        setLoading((prev: any) => ({ ...prev, topics: false }));
      }
    };
  ...
  const fetchContent = async (tagValue: string) => {
    try {
      ...
      if (combinedContent.length === 0) {
        setLoading((prev: any) => ({ ...prev, error: true }));
      }
  ...
  const handleTagPress = async (tag: { value: string }) => {
    try {
      ...
      setLoading((prev: any) => ({ ...prev, content: true, error: false }));
      setIsExperiencesVisible(true);
      setContentItems([]);
      await fetchContent(tag.value);
      setLoading((prev: any) => ({ ...prev, content: false }));
    } catch (error) {
      showToast("Failed to fetch content", "error");
      setLoading((prev: any) => ({ ...prev, content: false, error: true }));
      setSelectedTag(null);
      setContentItems([]);
